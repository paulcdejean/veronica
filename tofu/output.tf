...
output "gateway_token_command" {
  value       = "gcloud compute ssh ${google_compute_instance.openclaw.name} --project=${local.workspace.project_id} --zone=${google_compute_instance.openclaw.zone} --tunnel-through-iap --command='sudo sed -n s/^OPENCLAW_GATEWAY_TOKEN=//p /etc/openclaw/gateway.env'"
